...
public class Hooks {
	...
	public void TearDownTest(Scenario scenario) {
		...
		if (scenario.isFailed()) {
			try {
				Log.error("Failed page URL is " + driver.getCurrentUrl());
				TakesScreenshot ts = (TakesScreenshot) driver;
				File sourceLocation = ts.getScreenshotAs(OutputType.FILE);
				String screenShotPath = new File("").getAbsolutePath() + "//Screenshots//" +"TakeAway"+ System.currentTimeMillis()
						+ ".png";
				...
				logger.fail(scenario.getName(), MediaEntityBuilder.createScreenCaptureFromPath(screenShotPath).build());
			} catch (IOException e) {
